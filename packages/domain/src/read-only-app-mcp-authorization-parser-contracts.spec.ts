...
describe("FP-0146 Authorization parser contract and provider-selection proof", () => {
  ...
  it("accepts exact FP-0157, FP-0158, FP-0159 readiness, and FP-0160 implementation plans while FP-0161 remains absent", () => {
    ...
    const repoPathsWithoutFp0158 = repoPaths.filter(
      (path) =>
        path !== FP0158_READ_ONLY_MCP_EVIDENCE_APP_LOCAL_DEMO_BRIDGE_PLAN_PATH,
    );
    const repoPathsWithoutFp0159 = repoPaths.filter(
      (path) =>
        path !==
        FP0159_READ_ONLY_MCP_EVIDENCE_APP_LOCAL_PREVIEW_DEMO_UI_BRIDGE_READINESS_PLAN_PATH,
    );
    ...
    expect(repoPaths.filter((path) => /(^|\/)FP-0158/u.test(path))).toEqual([
      FP0158_READ_ONLY_MCP_EVIDENCE_APP_LOCAL_DEMO_BRIDGE_PLAN_PATH,
    ]);
    expect(verifyFp0158Absent(repoPathsWithoutFp0158)).toBe(true);
    expect(
    ...
    expect(
      verifyFp0158AbsentOrReadOnlyMcpEvidenceAppLocalDemoBridgePlan([
        ...repoPaths,
        "plans/FP-0158-runtime.md",
      ]),
    ).toBe(false);
    expect(
      verifyFp0158Absent([
        ...repoPathsWithoutFp0158,
        "plans/FP-0158-runtime.md",
      ]),
    ).toBe(false);
    expect(repoPaths.filter((path) => /(^|\/)FP-0159/u.test(path))).toEqual([
      FP0159_READ_ONLY_MCP_EVIDENCE_APP_LOCAL_PREVIEW_DEMO_UI_BRIDGE_READINESS_PLAN_PATH,
    ]);
    expect(verifyFp0159Absent(repoPathsWithoutFp0159)).toBe(true);
    expect(
      verifyFp0159AbsentOrReadOnlyMcpEvidenceAppLocalPreviewDemoUiBridgeReadinessPlan(
        repoPaths,
      ),
    ...
    expect(
      verifyFp0159AbsentOrReadOnlyMcpEvidenceAppLocalPreviewDemoUiBridgeReadinessPlan(
        [...repoPaths, "plans/FP-0159-runtime.md"],
      ),
    ).toBe(false);
    ...
    const repoPathsWithoutFp0161 = repoPaths.filter(
      (path) => !/(^|\/)FP-0161/u.test(path),
    );

    expect(verifyFp0161Absent(repoPathsWithoutFp0161)).toBe(true);
    expect(verifyFp0161Absent(repoPaths)).toBe(false);
    expect(
      verifyFp0161AbsentOrReadOnlyMcpEvidenceAppLocalPreviewDemoVisualQaAccessibilityPlan(
        repoPaths,
      ),
    ...
    expect(verifyFp0164Absent(repoPathsWithoutFp0164)).toBe(true);
    expect(verifyFp0164Absent(repoPaths)).toBe(false);
    expect(
      verifyFp0164AbsentOrReadOnlyMcpLocalAppsSdkResourceRegistrationPlan(
        repoPaths,
      ),
    ...
    expect(verifyFp0165Absent(repoPathsWithoutFp0165)).toBe(true);
    expect(verifyFp0165Absent(repoPaths)).toBe(false);
    expect(
      verifyFp0165AbsentOrReadOnlyMcpLocalRenderToolDescriptorReadinessPlan(
        repoPaths,
      ),
    ...
    expect(verifyFp0167Absent([...repoPaths, "plans/FP-0167-runtime.md"])).toBe(
      false,
    );
  });
