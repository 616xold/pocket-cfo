...
export const FP0149_AUTHORIZATION_PARSER_PURE_DOMAIN_IMPLEMENTATION_PLAN_PATH =
...
export const FP0151_AUTHORIZATION_PARSER_ROUTE_INTEGRATION_READINESS_PLAN_PATH =
  "plans/FP-0151-read-only-chatgpt-app-mcp-authorization-parser-route-integration-implementation-readiness.md";

export const FP0152_AUTHORIZATION_PARSER_FOLLOWUP_PLAN_PREFIX = "FP-0152";

export const FP0152_AUTHORIZATION_PARSER_ROUTE_INTEGRATION_IMPLEMENTATION_PLAN_PATH =
  "plans/FP-0152-read-only-chatgpt-app-mcp-authorization-parser-route-integration-implementation.md";

export const FP0153_AUTHORIZATION_PARSER_FOLLOWUP_PLAN_PREFIX = "FP-0153";

export const FP0153_AUTHORIZATION_PARSER_APP_CONSTRUCTION_WIRING_PLAN_PATH =
  "plans/FP-0153-read-only-chatgpt-app-mcp-authorization-parser-app-construction-wiring.md";

export const FP0154_AUTHORIZATION_PARSER_FOLLOWUP_PLAN_PREFIX = "FP-0154";

export const FP0154_AUTHORIZATION_PARSER_LOCAL_ADAPTER_CONSTRUCTION_READINESS_PLAN_PATH =
  "plans/FP-0154-read-only-chatgpt-app-mcp-authorization-parser-local-adapter-construction-readiness.md";

export const FP0155_AUTHORIZATION_PARSER_FOLLOWUP_PLAN_PREFIX = "FP-0155";

export const FP0155_AUTHORIZATION_PARSER_LOCAL_ADAPTER_IMPLEMENTATION_PLAN_PATH =
  "plans/FP-0155-read-only-chatgpt-app-mcp-authorization-parser-local-adapter-implementation.md";

export const FP0156_AUTHORIZATION_PARSER_FOLLOWUP_PLAN_PREFIX = "FP-0156";

export const FP0156_AUTHORIZATION_PARSER_LOCAL_ADAPTER_APP_CONSTRUCTION_INJECTION_PLAN_PATH =
...
export const FP0158_READ_ONLY_MCP_EVIDENCE_APP_LOCAL_DEMO_BRIDGE_PLAN_PATH =
...
export const FP0160_READ_ONLY_MCP_EVIDENCE_APP_LOCAL_PREVIEW_DEMO_UI_BRIDGE_IMPLEMENTATION_PLAN_PATH =
...
export const FP0163_READ_ONLY_MCP_LOCAL_APPS_SDK_RESOURCE_SKELETON_PLAN_PATH =
  "plans/FP-0163-read-only-chatgpt-app-mcp-local-apps-sdk-resource-skeleton.md";

export const FP0164_READ_ONLY_MCP_FOLLOWUP_PLAN_PREFIX = "FP-0164";

export const FP0164_READ_ONLY_MCP_LOCAL_APPS_SDK_RESOURCE_REGISTRATION_PLAN_PATH =
...
export function verifyFp0149Absent(repoPaths: readonly string[]) {
  return verifyFp0149AbsentOrAuthorizationParserPureDomainImplementationPlan(
    repoPaths,
  );
}

export function verifyFp0149AbsentOrAuthorizationParserPureDomainImplementationPlan(
  repoPaths: readonly string[],
) {
  const hits = fpPlanHits(
    repoPaths,
    FP0149_AUTHORIZATION_PARSER_FOLLOWUP_PLAN_PREFIX,
  );
  if (hits.length === 0) return true;
...
export function verifyFp0150Absent(repoPaths: readonly string[]) {
  return (
    fpPlanHits(repoPaths, FP0150_AUTHORIZATION_PARSER_FOLLOWUP_PLAN_PREFIX)
      .length === 0
  );
}
...
export function verifyFp0151AbsentOrAuthorizationParserRouteIntegrationReadinessPlan(
  repoPaths: readonly string[],
) {
  const hits = fpPlanHits(
    repoPaths,
    FP0151_AUTHORIZATION_PARSER_FOLLOWUP_PLAN_PREFIX,
  );
  if (hits.length === 0) return true;
...
export function verifyFp0152Absent(repoPaths: readonly string[]) {
  return (
    fpPlanHits(repoPaths, FP0152_AUTHORIZATION_PARSER_FOLLOWUP_PLAN_PREFIX)
      .length === 0
  );
}

export function verifyFp0152AbsentOrAuthorizationParserRouteIntegrationImplementationPlan(
  repoPaths: readonly string[],
) {
  const hits = fpPlanHits(
    repoPaths,
    FP0152_AUTHORIZATION_PARSER_FOLLOWUP_PLAN_PREFIX,
  );
  if (hits.length === 0) return true;
...
export function verifyFp0153Absent(repoPaths: readonly string[]) {
  return (
    fpPlanHits(repoPaths, FP0153_AUTHORIZATION_PARSER_FOLLOWUP_PLAN_PREFIX)
      .length === 0
  );
}

export function verifyFp0153AbsentOrAuthorizationParserAppConstructionWiringPlan(
  repoPaths: readonly string[],
) {
  const hits = fpPlanHits(
    repoPaths,
    FP0153_AUTHORIZATION_PARSER_FOLLOWUP_PLAN_PREFIX,
  );
  if (hits.length === 0) return true;
...
export function verifyFp0154Absent(repoPaths: readonly string[]) {
  return (
    fpPlanHits(repoPaths, FP0154_AUTHORIZATION_PARSER_FOLLOWUP_PLAN_PREFIX)
      .length === 0
  );
}

export function verifyFp0154AbsentOrAuthorizationParserLocalAdapterConstructionReadinessPlan(
  repoPaths: readonly string[],
) {
  const hits = fpPlanHits(
    repoPaths,
    FP0154_AUTHORIZATION_PARSER_FOLLOWUP_PLAN_PREFIX,
  );
  if (hits.length === 0) return true;
...
export function verifyFp0155Absent(repoPaths: readonly string[]) {
  return (
    fpPlanHits(repoPaths, FP0155_AUTHORIZATION_PARSER_FOLLOWUP_PLAN_PREFIX)
      .length === 0
  );
}

export function verifyFp0155AbsentOrAuthorizationParserLocalAdapterImplementationPlan(
  repoPaths: readonly string[],
) {
  const hits = fpPlanHits(
    repoPaths,
    FP0155_AUTHORIZATION_PARSER_FOLLOWUP_PLAN_PREFIX,
  );
  if (hits.length === 0) return true;
...
export function verifyFp0156Absent(repoPaths: readonly string[]) {
  return verifyFp0156AbsentOrAuthorizationParserLocalAdapterAppConstructionInjectionPlan(
    repoPaths,
  );
}

export function verifyFp0156AbsentOrAuthorizationParserLocalAdapterAppConstructionInjectionPlan(
  repoPaths: readonly string[],
) {
  const hits = fpPlanHits(
    repoPaths,
    FP0156_AUTHORIZATION_PARSER_FOLLOWUP_PLAN_PREFIX,
  );
  if (hits.length === 0) return true;
...
export function verifyFp0158AbsentOrReadOnlyMcpEvidenceAppLocalDemoBridgePlan(
  repoPaths: readonly string[],
) {
  const hits = fpPlanHits(
    repoPaths,
    FP0158_AUTHORIZATION_PARSER_FOLLOWUP_PLAN_PREFIX,
  );
  if (hits.length === 0) return true;

  return (
    hits.length === 1 &&
    hits[0] === FP0158_READ_ONLY_MCP_EVIDENCE_APP_LOCAL_DEMO_BRIDGE_PLAN_PATH
  );
}

export function verifyFp0159Absent(repoPaths: readonly string[]) {
  return (
...
export function verifyFp0160AbsentOrReadOnlyMcpEvidenceAppLocalPreviewDemoUiBridgeImplementationPlan(
  repoPaths: readonly string[],
) {
  const hits = fpPlanHits(
    repoPaths,
...
export function verifyFp0161Absent(repoPaths: readonly string[]) {
  return (
    fpPlanHits(
      repoPaths,
...
export function verifyFp0163AbsentOrReadOnlyMcpLocalAppsSdkResourceSkeletonPlan(
  repoPaths: readonly string[],
) {
  const hits = fpPlanHits(
    repoPaths,
    FP0163_READ_ONLY_MCP_FOLLOWUP_PLAN_PREFIX,
  );
...
export function verifyFp0164Absent(repoPaths: readonly string[]) {
  return (
    fpPlanHits(repoPaths, FP0164_READ_ONLY_MCP_FOLLOWUP_PLAN_PREFIX).length ===
    0
  );
}

export function verifyFp0164AbsentOrReadOnlyMcpLocalAppsSdkResourceRegistrationPlan(
  repoPaths: readonly string[],
) {
  const hits = fpPlanHits(
    repoPaths,
    FP0164_READ_ONLY_MCP_FOLLOWUP_PLAN_PREFIX,
  );
