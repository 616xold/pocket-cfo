...
const DEFAULT_COMPANY_KEY = "local-finance-account-bridge-smoke-company";
const DEFAULT_COMPANY_NAME = "Local Finance Account Bridge Smoke Company";
const DEFAULT_CREATED_BY = "finance-account-bridge-smoke";
...
function buildFixture(input) {
  const seedText = JSON.stringify(
    {
      ...
      note: "Seed snapshot for the packaged finance account-bridge smoke.",
      requestedBy: "finance_account_bridge_smoke",
      runTag: input.runTag,
    },
    null,
    2,
  ...
  return {
    ...
    createdBy: input.createdBy,
    source: {
      sourceName: `Account bridge smoke ${input.runTag}`,
      capturedAt: "2026-04-11T00:00:00.000Z",
      seed: {
        body: Buffer.from(`${seedText}\n`, "utf8"),
        mediaType: "application/json",
        originalFileName: `finance-account-bridge-seed-${input.runTag}.json`,
      },
    },
    chartOfAccounts: {
...
async function main() {
  ...
  try {
    ...
    const accountBridge = await injectJson(app, {
      expectedStatus: 200,
      method: "GET",
      url: `/finance-twin/companies/${fixture.companyKey}/reconciliation/trial-balance-vs-general-ledger/account-bridge`,
    });
    ...
    const activityLineage = await injectJson(app, {
      expectedStatus: 200,
      method: "GET",
      url: `/finance-twin/companies/${fixture.companyKey}/general-ledger/accounts/${archivedClearingRow?.ledgerAccount.id ?? ""}/lineage?${new URLSearchParams({
        syncRunId:
          archivedClearingRow?.activityLineageRef?.syncRunId ??
          generalLedgerSync.syncRun.id,
      }).toString()}`,
    });
      ...
      JSON.stringify(
        {
          ...
          company: accountBridge.company,
          bridgeReadiness: accountBridge.bridgeReadiness,
          coverageSummary: accountBridge.coverageSummary,
          accounts: accountBridge.accounts.map((account) => ({
          ...
          limitations: accountBridge.limitations,
        },
        null,
        2,
      ),
...
async function createSharedSource(app, fixture) {
  ...
  return injectJson(app, {
    ...
    payload: {
      createdBy: fixture.createdBy,
      description: "Packaged finance account-bridge smoke source.",
      kind: "dataset",
...
async function writeSeedSnapshot(seed) {
  const directory = await mkdtemp(
    join(tmpdir(), "pocket-cfo-finance-account-bridge-smoke-"),
  );
...
async function injectJson(app, input) {
  ...
  if (response.statusCode !== input.expectedStatus) {
    throw new Error(
      `${MODULE_PATH} expected ${input.expectedStatus} from ${input.method} ${input.url}, got ${response.statusCode}: ${response.body}`,
    );
  }
