...
function verifyParserScope() {
  ...
  return {
    ...
    noRawAuthorizationHeaderOutput:
      forbiddenOutputFields.every(
        (field) => !sanitizedOutputFields.includes(field),
      ),
    noRawTokenOutput:
...
function verifyFp0149PlanningTextRequiredTopics(planText) {
  ...
  return {
    ...
    fp0150Boundary:
      normalized.includes("future fp-0150 may open only route-integration sequencing") &&
      normalized.includes("route integration itself remains blocked"),
    oauthAuthBlocked:
      normalized.includes("oauth/session/auth middleware remains blocked"),
    outputBoundary:
    ...
    providerSelectionDeferred:
      normalized.includes("provider selection remains provider-neutral/deferred"),
    publicAppBlocked:
      normalized.includes("public chatgpt app demo/submission remains blocked"),
    rawMaterialBlocked:
    ...
    tokenValidationBlocked:
      normalized.includes("production token validation remains blocked"),
  };
}
