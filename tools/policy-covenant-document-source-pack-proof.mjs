...
import { pocketCfoPolicyCovenantDocumentSourcePack } from "../packages/stack-packs/src/index.ts";
import { loadNearestEnvFile } from "./m2-exit-utils.mjs";

const CREATED_BY = "policy-covenant-document-source-pack-proof";
const COMPANY_NAME = "Demo Policy/Covenant Document Source Pack";
...
async function main() {
  ...
  try {
    ...
    assertJsonEqual(
      "normalized source/wiki/policy posture",
      actual,
      fixture.expected,
    );
    ...
    console.log(
      JSON.stringify({
        sourcePackId: pocketCfoPolicyCovenantDocumentSourcePack.id,
        ...
        policyPageVerified: true,
        policyCorpusVerified: true,
...
function assertManifestBoundary(expected, sourceFiles) {
  ...
  assertJsonEqual(
    "source-pack roles",
    pocketCfoPolicyCovenantDocumentSourcePack.sourceRoles,
    expected.sourceRolesPresent,
  );
  assertJsonEqual(
    "document roles",
    pocketCfoPolicyCovenantDocumentSourcePack.documentRoles,
    expected.documentRolesPresent,
  );
  assertJsonEqual(
    "source kinds",
    pocketCfoPolicyCovenantDocumentSourcePack.sourceKinds,
    expected.sourceKindsPresent,
  );
  assertJsonEqual(
    "media types",
    pocketCfoPolicyCovenantDocumentSourcePack.mediaTypes,
  ...
  if (
    pocketCfoPolicyCovenantDocumentSourcePack.sourceRoles.length !== 1 ||
    pocketCfoPolicyCovenantDocumentSourcePack.sourceRoles[0] !==
      "policy_document"
  ) {
    throw new Error("F6W source pack must be limited to policy_document");
  }

  if (
    !pocketCfoPolicyCovenantDocumentSourcePack.sourceFiles.every(
      (sourceFile) =>
        sourceFile.sourceKind === "document" &&
        sourceFile.documentRole === "policy_document" &&
    ...
    throw new Error(
      "F6W source files must be markdown/plain-text policy documents without extractor keys",
    );
  }
  ...
  for (const forbidden of FORBIDDEN_MANIFEST_TARGETS) {
    if (Object.hasOwn(pocketCfoPolicyCovenantDocumentSourcePack, forbidden)) {
  ...
  for (const sourceFile of fixture.sourceFiles) {
    ...
    const created = await injectJson(app, {
      ...
      payload: {
        ...
        description: `F6W policy/covenant document source-pack source for ${sourceFile.path}.`,
        kind: sourceFile.sourceKind,
        name: `F6W ${basename(sourceFile.path)} ${fixture.expected.companyKey}`,
        snapshot: {
          capturedAt,
...
async function readWikiPosture(app, fixture, registered) {
  ...
  const sourceCoveragePage = await readWikiPage(app, companyKey, "sources/coverage");
  const policyCorpusPage = await readWikiPage(
    app,
    companyKey,
  ...
  for (const sourceFile of fixture.sourceFiles) {
    ...
    policyPages[sourceFile.path] = await readWikiPage(
      app,
      companyKey,
      `policies/${sourceId}`,
    );
...
function normalizePosture(input) {
  return {
    ...
    sourceRolesPresent: pocketCfoPolicyCovenantDocumentSourcePack.sourceRoles,
    documentRolesPresent:
      pocketCfoPolicyCovenantDocumentSourcePack.documentRoles,
    sourceKindsPresent: pocketCfoPolicyCovenantDocumentSourcePack.sourceKinds,
    mediaTypesPresent: pocketCfoPolicyCovenantDocumentSourcePack.mediaTypes,
    ...
    sourceListPosture: {
      ...
      allSourcesBoundAsPolicyDocument: currentSourceSummaries(input).every(
        (entry) => entry.binding.documentRole === "policy_document",
      ),
      listLimitationsPresent: input.wikiReads.sourceList.limitations.length > 0,
    },
    policyPagePosture: input.sourceFiles.map((sourceFile) =>
      normalizePolicyPagePosture(input, sourceFile),
    ),
    sourceDigestPosture: input.sourceFiles.map((sourceFile) =>
      normalizeSourceDigestPosture(input, sourceFile),
    ),
    policyCorpusPosture: normalizePolicyCorpusPosture(input),
    ...
    freshnessSourceBackedStateByRole: {
      policy_document: {
        freshnessState: mergeFreshnessStates(
          input.sourceFiles.map(
            (sourceFile) =>
              input.wikiReads.policyPages[sourceFile.path].freshnessSummary
                .state,
          ),
        ),
    ...
    limitations: pocketCfoPolicyCovenantDocumentSourcePack.limitations.map(
...
function normalizeSourceRegistrationPosture(input) {
  return {
    ...
    uploadedSourceFileCount: Object.values(input.registered).filter(
      (entry) => Boolean(entry.uploaded?.sourceFile?.id),
    ).length,
...
function normalizeBindingPosture(input) {
  return {
    ...
    policyDocumentBindingCount: Object.values(input.bindings).filter(
      (bindingView) =>
        bindingView.source.binding.documentRole === "policy_document",
    ).length,
...
function normalizeExtractPosture(input, sourceFile) {
  ...
  return {
    ...
    thresholdLinePresent: (extract.extractedText ?? "").includes(
      "Pocket CFO threshold:",
    ),
...
function normalizePolicyPagePosture(input, sourceFile) {
  const page = input.wikiReads.policyPages[sourceFile.path];

  return {
    path: sourceFile.path,
    pagePresent: Boolean(page.page?.pageKey),
    pageKind: page.page.pageKind,
    ownershipKind: page.page.ownershipKind,
    ...
    deterministicExtractSectionPresent: page.page.markdownBody.includes(
      "## Deterministic Policy Extract",
    ),
    relatedPolicyHistoryPresent: page.page.markdownBody.includes(
...
function normalizePolicyCorpusPosture(input) {
  const page = input.wikiReads.policyCorpusPage;
  const currentPolicyPageKeys = input.sourceFiles.map(
    (sourceFile) => input.wikiReads.policyPages[sourceFile.path].page.pageKey,
  );
  const currentSourceDigestPageKeys = input.sourceFiles.map(
    (sourceFile) =>
      input.wikiReads.sourceDigestPages[sourceFile.path].page.pageKey,
  );
  const policyPageLinks = page.links.filter((link) =>
    currentPolicyPageKeys.includes(link.toPageKey),
  );
  ...
  return {
    ...
    pageKind: page.page.pageKind,
    freshnessState: page.freshnessSummary.state,
    policyPageCount: policyPageLinks.length,
    policyPageLinksPresent: currentPolicyPageKeys.every((pageKey) =>
      page.links.some((link) => link.toPageKey === pageKey),
    ),
    sourceDigestLinksPresent: currentSourceDigestPageKeys.every((pageKey) =>
      page.links.some((link) => link.toPageKey === pageKey),
    ),
    limitationsPresent: page.limitations.length > 0,
  };
...
function normalizeCompanySummaryPosture(input) {
  ...
  const policyPageMinimum =
    input.expected.companySummaryPosture.policyPageCountMinimum;
  ...
  if (
    input.wikiReads.companySummary.pageCountsByKind.policy < policyPageMinimum
  ) {
    throw new Error(
      `Expected at least ${policyPageMinimum} policy pages, received ${input.wikiReads.companySummary.pageCountsByKind.policy}`,
    );
  }
  ...
  return {
    ...
    policyPageCountMinimum: policyPageMinimum,
...
function normalizeLineagePresence(input, sourceFile) {
  const sourceSummary = sourceSummaryFor(input, sourceFile);
  const policyPage = input.wikiReads.policyPages[sourceFile.path];
  ...
  return {
    ...
    policyPageLineagePresent: policyPage.refs.some(
      (ref) => ref.targetKind === "source_snapshot",
    ),
    policyCorpusLineagePresent:
      input.wikiReads.policyCorpusPage.links.some((link) =>
        link.toPageKey.startsWith("policies/"),
      ),
...
function normalizeLimitationPresence(input, sourceFile) {
  ...
  return {
    path: sourceFile.path,
    sourceLimitationsPresent: sourceSummary.limitations.length > 0,
    policyPageLimitationsPresent:
      input.wikiReads.policyPages[sourceFile.path].limitations.length > 0,
    policyCorpusLimitationsPresent:
      input.wikiReads.policyCorpusPage.limitations.length > 0,
...
function normalizeManifestSourceFiles() {
  return pocketCfoPolicyCovenantDocumentSourcePack.sourceFiles.map(
    (sourceFile) => ({
