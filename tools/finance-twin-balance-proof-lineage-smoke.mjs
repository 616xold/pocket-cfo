...
const DEFAULT_COMPANY_KEY = "local-finance-balance-proof-lineage-smoke-company";
const DEFAULT_COMPANY_NAME =
  "Local Finance Balance Proof Lineage Smoke Company";
const DEFAULT_CREATED_BY = "finance-balance-proof-lineage-smoke";
...
function buildFixture(input) {
  const seedText = JSON.stringify(
    {
      ...
      note: "Seed snapshot for the packaged finance balance-proof lineage smoke.",
      requestedBy: "finance_balance_proof_lineage_smoke",
      runTag: input.runTag,
    },
    null,
    2,
  ...
  return {
    ...
    createdBy: input.createdBy,
    source: {
      sourceName: `Balance proof lineage smoke ${input.runTag}`,
      capturedAt: "2026-04-12T00:00:00.000Z",
      seed: {
        body: Buffer.from(`${seedText}\n`, "utf8"),
        mediaType: "application/json",
        originalFileName: `finance-balance-proof-lineage-seed-${input.runTag}.json`,
      },
    },
    chartOfAccounts: {
...
async function main() {
  ...
  try {
    ...
    if (!cashAccount?.balanceProofLineageRef) {
      throw new Error(
        `Expected a proof-bearing account row to expose balanceProofLineageRef, received: ${JSON.stringify(
          cashAccount,
          null,
          2,
        )}`,
    ...
    const balanceProof = await injectJson(app, {
      expectedStatus: 200,
      method: "GET",
      url: `/finance-twin/companies/${fixture.companyKey}/general-ledger/accounts/${cashAccount.ledgerAccount.id}/balance-proof`,
    });

    if (
      balanceProof.proof?.record?.id !== cashAccount.balanceProofLineageRef.targetId ||
      balanceProof.proof?.balanceProof?.proofBasis !==
        "source_backed_balance_field" ||
      balanceProof.lineage?.recordCount !== 1
    ) {
      throw new Error(
        `Expected persisted proof row and lineage for account 1000, received: ${JSON.stringify(
          {
            balanceProof,
            cashAccount,
            generalLedgerSync,
    ...
    const proofLineage = await injectJson(app, {
      expectedStatus: 200,
      method: "GET",
      url: `/finance-twin/companies/${fixture.companyKey}/lineage/general_ledger_balance_proof/${cashAccount.balanceProofLineageRef.targetId}?${new URLSearchParams(
        {
          syncRunId: cashAccount.balanceProofLineageRef.syncRunId,
        },
      ).toString()}`,
    });
      ...
      JSON.stringify(
        {
          ...
          company: balanceProof.company,
          cashAccount: {
            accountCode: cashAccount.ledgerAccount.accountCode,
            balanceProofLineageRef: cashAccount.balanceProofLineageRef,
...
async function createSharedSource(app, fixture) {
  ...
  return injectJson(app, {
    ...
    payload: {
      createdBy: fixture.createdBy,
      description: "Packaged finance balance-proof lineage smoke source.",
      kind: "dataset",
...
async function writeSeedSnapshot(seed) {
  const directory = await mkdtemp(
    join(tmpdir(), "pocket-cfo-finance-balance-proof-lineage-smoke-"),
  );
