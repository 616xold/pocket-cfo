...
[plans/FP-0093-read-only-chatgpt-app-mcp-premium-ui-preview-route-master-plan.md](../plans/FP-0093-read-only-chatgpt-app-mcp-premium-ui-preview-route-master-plan.md) is the active V2M docs-and-plan plus proof-gate compatibility record for a future local read-only premium UI preview route. It plans one future local preview route boundary only and keeps route code, app routes, API routes, backend routes, endpoints, remote MCP, Apps SDK iframe/UI resources, OAuth, app submission, public app implementation, schema, migrations, package scripts, eval datasets, fixtures, sample data, source packs, OpenAI API/model calls, provider/certification/deployment, external communications, source mutation, finance writes, generated product prose, runtime-Codex finance output, autonomous action, screenshots, images, and public app assets out of scope.
...
- ChatGPT App/MCP implementation beyond the active FP-0093 docs-only local preview-route plan, the shipped FP-0092 local composition/accessibility foundation, the shipped FP-0091 local component-only foundation, the shipped FP-0090 docs-only UI implementation readiness plan, the shipped FP-0089 design-system readiness plan, the shipped FP-0088 UI/security readiness plan, the shipped FP-0087 local proof-only contract and descriptor/envelope foundation, and the shipped local/internal FP-0082 contract
