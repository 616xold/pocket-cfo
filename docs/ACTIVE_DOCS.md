...
[plans/FP-0086-benchmark-community-pack-foundation.md](../plans/FP-0086-benchmark-community-pack-foundation.md) is the active V2F benchmark/community pack foundation plan. It is docs-and-plan only until a later implementation thread continues it. It narrows V2F to SafeDemoDataPolicy-first, docs/proof-only benchmark/community manifest contracts, read-only benchmark task taxonomy, and no-runtime/no-real-finance-data boundaries. It does not add implementation, code, UI, routes, schema, migrations, package scripts, smoke aliases, eval datasets, fixtures, sample data, source packs, source-pack mutations, OpenAI API/model calls, public ChatGPT App/MCP, Apps SDK UI, OAuth, app submission, provider behavior, certification, delivery, deployment, external communications, source mutation, finance writes, generated advice, runtime-Codex finance output, or autonomous action.
...
- V2F implementation beyond the active FP-0086 docs/proof-only benchmark/community foundation plan
