...
5. `WORKFLOW.md`
6. `plans/ROADMAP.md`
7. the unfinished `plans/FP-*.md` file if one exists; right now `plans/FP-0072-board-lender-document-source-pack-foundation.md` is the active F6Y implementation-ready contract for one future board/lender document source-pack foundation. It is docs-and-plan only: F6Y implementation has not started, no FP-0073 exists, and it adds no code, route, schema, migration, package script, smoke alias, eval dataset, fixture, UI, runtime-Codex, delivery, provider, outbox, report, approval, certification, monitor family, discovery family, mission, generated prose, source mutation outside future proof setup, finance write, or autonomous action. `plans/FP-0071-policy-covenant-document-source-pack-foundation.md` remains the shipped F6W record with direct proof `pnpm exec tsx tools/policy-covenant-document-source-pack-proof.mjs`. `plans/FP-0070-close-control-certification-safety-foundation.md` remains the shipped non-certifying F6T record. `plans/FP-0069-ledger-reconciliation-source-pack-foundation.md` remains the shipped F6U record. FP-0050 through FP-0071 remain shipped F6 records, and `plans/FP-0049-board-packet-circulation-note-reset-and-effective-record-hardening.md` remains the latest shipped F5C4I record. F6V, F6X, F6Z, and later planning must wait for future Finance Plans. Treat older F5 and F4 records as shipped history unless a new plan names a concrete truthfulness gap.
