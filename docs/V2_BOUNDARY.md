...
- **V2G read-only ChatGPT App/MCP master plan**: active through FP-0087 as docs-and-plan-only planning for future read-only ChatGPT App/MCP concepts, allowlists, forbidden tools, refusal posture, prompt-injection/privacy/no-runtime boundaries, proof posture, and wait conditions. It adds no implementation, code, UI, routes, endpoints, remote MCP server, Apps SDK iframe, OAuth, app submission, schema, migrations, package scripts, eval datasets, fixtures, sample data, public demo data, source-pack behavior, OpenAI API/model calls, vector/file-search integration, OCR, PageIndex, provider behavior, certification, delivery, deployment, external communications, source mutation, finance writes, generated product prose, runtime-Codex finance output, or autonomous action.

## Distribution And Precision Decisions

- ChatGPT App/MCP is worth planning as a read-only evidence app after EvidenceIndex, V2B precision proof, the shipped FP-0082 local/internal tool contract, the shipped FP-0083 OSS/security/privacy/self-host/demo baseline, the shipped V2E bounded-orchestration posture, and the shipped V2F no-real-finance-data benchmark/community posture. FP-0087 is that planning layer only. MCP server implementation, ChatGPT App implementation, Apps SDK UI, remote deployment, OAuth, and app publication remain future implementation work.
- iOS is postponed.
