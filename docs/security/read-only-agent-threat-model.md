...
FP-0083 documents the shipped V2C local/internal evidence-tool contract. FP-0085 through FP-0124 ship the bounded read-only ChatGPT App/MCP planning, proof, route-input, metadata-builder, OAuth-deferral, remote-host-deferral, security, and evidence-dispatch boundaries described in the active Finance Plans. FP-0125 ships one local-only/read-only protected-resource metadata route at GET `/.well-known/oauth-protected-resource/mcp`, registered only through explicit valid FP-0123 route-input evidence dependency, with default `buildApp()` route absence and unchanged `/mcp` behavior. FP-0126 ships only a docs-and-plan/proof-gate compatibility master plan for future WWW-Authenticate `resource_metadata` auth-challenge sequencing. FP-0127 ships only local/proof-only/read-only WWW-Authenticate auth-challenge domain contracts and direct proof tooling. FP-0128 ships only local/proof-only/read-only token-validation failure readiness contracts and direct proof tooling. FP-0129 ships only a docs-and-plan/proof-gate compatibility master plan for future WWW-Authenticate challenge implementation sequencing. FP-0130 ships only a local-only/read-only explicit-dependency missing-token `WWW-Authenticate` challenge seam for existing `POST /mcp`, with default `buildApp()` and default `/mcp` behavior unchanged and Authorization-present requests failing closed without token parsing or validation. FP-0131 ships only a docs-and-plan/proof-gate compatibility plan for token-validation and invalid-token runtime sequencing. FP-0132 ships only local/proof-only/read-only token-validation runtime contracts and proof tooling. FP-0133 ships only local/proof-only/read-only token-validation test-double contracts and proof tooling. FP-0134 ships only a local-only/read-only synthetic token-validation test-double evaluator. FP-0135 ships only a docs-and-plan/proof-gate invalid-token WWW-Authenticate challenge sequencing plan. FP-0136 ships only local/proof-only/read-only invalid-token challenge contracts and proof tooling. FP-0137 ships only a docs-and-plan/proof-gate invalid-token challenge implementation-readiness plan and proof-gate compatibility bridge. FP-0138 ships only a docs-and-plan/proof-gate token-validation runtime implementation-planning plan and proof-gate compatibility bridge. FP-0139 ships only local/proof-only/read-only token-validation result-envelope contracts, builder/evaluator logic, focused specs, and direct proof tooling for sanitized descriptors. FP-0140 ships only docs-and-plan/proof-gate invalid-token challenge implementation planning after FP-0139. FP-0141 ships only local-only/read-only invalid-token challenge mapping from sanitized FP-0139 result envelopes through an explicit `/mcp` dependency.

These records do not start a public MCP server, public ChatGPT App, Apps SDK UI/resource implementation, root or dual protected-resource metadata route, WWW-Authenticate behavior beyond the FP-0130 explicit missing-token seam and FP-0141 explicit invalid-token sanitized-envelope seam, test-double runtime consumption, route consumption of test doubles, OAuth/token/session/auth middleware implementation, production token validation runtime, token parsing runtime, JWT decoding, token introspection, app submission, remote deployment, deployment config, hidden/default evidence service construction, OpenAI API/model calls, OpenAI file-search/vector integration, provider calls, external communications, source mutation, finance writes, generated product prose, runtime-Codex finance output, FP-0142, or autonomous action. FP-0134 adds only local synthetic descriptor evaluation for future validation tests, FP-0135 adds only sequencing, FP-0136 adds only proof contracts, FP-0137 adds only readiness planning/proof-gate compatibility, FP-0138 adds only runtime implementation planning/proof-gate compatibility, FP-0139 adds only proof-mode result-envelope building, FP-0140 adds only implementation planning/proof-gate compatibility, and FP-0141 adds only local invalid-token challenge mapping from sanitized result envelopes.
