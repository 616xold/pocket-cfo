...
- `FP-0062` is now the active F6M docs-and-plan contract for internal delivery-readiness only; first implementation must derive readiness from shipped stored/read posture and preserve source lineage, freshness, limitations, proof posture, human-review next steps, and explicit no-send/no-provider/no-outbox/no-approval/no-report/no-runtime/no-action boundaries
