...
- `FP-0073` is now the active implementation-ready F6Z final F6/v1 exit audit and handoff contract; it is docs-and-validation-only and does not add source ingest, CFO Wiki compiler behavior, product runtime behavior, routes, schema, migrations, package scripts, smoke aliases, eval datasets, fixtures, provider integrations, outbox sends, report release, approvals, actual certification, runtime-codex, generated prose, source mutation, finance writes, or autonomous action
