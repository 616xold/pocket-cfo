...
`FP-0059` is the active F6J implementation-ready contract:
...
- the planned output is one deterministic internal operator readiness result or read model with bounded attention items carrying evidence basis, source lineage or proof reference, freshness or missing-source posture, limitations, proof posture, status, and a human-review next step
- F6J must not add source mutations, monitor families, discovery families, monitor reruns, missions, routes, schema, migrations, package scripts, eval datasets, fixtures, runtime-codex writeups, delivery, notification providers, outbox sends, email, Slack, SMS, webhooks, approvals, reports, accounting writes, bank writes, tax filings, legal or policy advice, payment instructions, collection instructions, customer-contact instructions, autonomous remediation, F6K, or FP-0060 in this docs-and-plan slice
