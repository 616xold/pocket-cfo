...
- `FP-0064` is now the active F6O contract for one receivables/payables source-pack foundation only; it plans source roles limited to `receivables_aging` and `payables_aging`, extractor keys limited to `receivables_aging_csv` and `payables_aging_csv`, one fixture/manifest/proof-oriented future slice, no package script or root smoke alias unless amended, no product runtime behavior, no source mutation, and no monitor/discovery family expansion
