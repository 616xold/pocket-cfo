...
- `FP-0072` is now the active F6Y planning contract for one future board/lender document source-pack foundation only; it may later add one immutable checked-in markdown/plain-text board/lender document fixture family, one source-pack manifest, one normalized expected source/wiki posture, and one direct proof through existing source registry and CFO Wiki bind/compile/read routes only, but it adds no implementation in this docs slice and must not add routes, schema, migrations, package scripts, smoke aliases, eval datasets, UI, monitor/discovery families, runtime-codex, delivery, providers, outbox sends, reports, board packets, lender updates, approvals, certification, generated prose, finance writes, legal/policy advice, source mutation outside future proof setup, or autonomous action
