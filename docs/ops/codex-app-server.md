...
`plans/FP-0061-source-pack-expansion-foundation.md` is the active F6L implementation-ready contract for one future bank/card source-pack foundation.
F6L remains runtime-free and delivery-free: it should prove checked-in bank-account-summary and card-expense source-pack posture through existing source registry and Finance Twin routes only, without asking the Codex App Server to interpret source truth, draft source-pack prose, generate monitor findings, create reports, send notifications, or take finance actions.

The Codex App Server should not draft monitoring findings, run natural-language autonomous monitoring, create investigation writeups, draft close/control checklist prose, draft acknowledgement prose, draft source-pack truth, draft notification messages, send notifications, publish messages, create payment instructions, recommend vendor payments, create collection instructions, interpret policy or legal obligations, or perform remediation in F6A, F6B, F6C, F6D, F6E, F6F, F6G, F6H, F6I, F6J, F6K, or F6L.
