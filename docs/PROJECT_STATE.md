...
- **V2AA read-only ChatGPT App/MCP local Fastify route adapter shell**: shipped through FP-0107 as exactly one local/control-plane `POST /mcp` protocol shell, with `initialize`, `ping`, `notifications/initialized`, `tools/list`, structured JSON-RPC errors, and fail-closed `tools/call`, and with no OAuth/token/session implementation, remote MCP deployment, Apps SDK iframe/resource registration, public app behavior, app submission, OpenAI API/model call, screenshot, generated image, public asset, listing copy, app-submission artifact, source mutation, finance write, provider call, external communication, generated finance advice, runtime-Codex finance output, autonomous action, or FP-0108.

## Latest Shipped V2 Plan

- **V2AA read-only ChatGPT App/MCP local Fastify route adapter shell**: shipped through FP-0107.

## Active V2 Plan

- No unfinished V2 Finance Plan is open after [FP-0107](../plans/FP-0107-read-only-chatgpt-app-mcp-local-fastify-mcp-route-adapter-foundation.md) closeout. Public app implementation, real read-only evidence tool dispatch, route/UI implementation beyond the FP-0107 local `/mcp` shell and FP-0094/FP-0096/FP-0097 single local preview-route boundary, OAuth/token/session implementation, remote MCP deployment, Apps SDK UI resources, app submission, provider/certification/delivery/deployment, and V2G expansion beyond the shipped local proof-only contracts, descriptor/envelope foundation, FP-0098 readiness/security/submission-boundary plan, FP-0099 security threat-model/platform-boundary plan, FP-0100 local security boundary contracts, FP-0101 implementation sequencing/platform-readiness plan, FP-0102 endpoint/OAuth/remote-MCP architecture plan, FP-0103 endpoint architecture proof-contract foundation, FP-0104 endpoint implementation readiness plan, FP-0105 endpoint route ownership proof-contract foundation, FP-0106 protocol envelope/tool-dispatch proof-contract foundation, and FP-0107 local route-adapter shell still require a later named Finance Plan.
