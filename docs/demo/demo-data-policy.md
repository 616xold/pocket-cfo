...
Pocket CFO demo data must be synthetic-only unless a future Finance Plan names and proves a stricter reviewed exception. FP-0086 ships SafeDemoDataPolicy as the first V2F benchmark/community gate, and the read-only ChatGPT App/MCP sequence through FP-0124 keeps demo/public assets out of scope. FP-0125 ships only a local-only/read-only explicit-dependency protected-resource metadata route at GET `/.well-known/oauth-protected-resource/mcp`. FP-0126 ships only a docs-and-plan/proof-gate compatibility master plan for future WWW-Authenticate `resource_metadata` auth-challenge sequencing. FP-0127 ships only local/proof-only/read-only WWW-Authenticate auth-challenge contracts and direct proof tooling. FP-0128 ships only local/proof-only/read-only token-validation failure readiness contracts and direct proof tooling. FP-0129 ships only a docs-and-plan/proof-gate compatibility master plan for future WWW-Authenticate challenge implementation sequencing. FP-0130 ships only a local-only/read-only explicit-dependency missing-token `WWW-Authenticate` challenge seam for existing `POST /mcp`. FP-0131 ships only a docs-and-plan/proof-gate compatibility plan for token-validation and invalid-token runtime sequencing. FP-0132 ships only local/proof-only/read-only token-validation runtime contracts and proof tooling. FP-0133 ships only local/proof-only/read-only token-validation test-double contracts and proof tooling. FP-0134 ships only a local-only/read-only synthetic token-validation test-double evaluator with no fixture or dataset addition. FP-0135 ships only a docs-and-plan/proof-gate invalid-token WWW-Authenticate challenge sequencing plan. FP-0136 ships only local/proof-only/read-only invalid-token challenge contracts and proof tooling. FP-0137 ships only a docs-and-plan/proof-gate invalid-token challenge implementation-readiness plan and proof-gate compatibility bridge. FP-0138 ships only a docs-and-plan/proof-gate token-validation runtime implementation-planning plan and proof-gate compatibility bridge. FP-0139 ships only local/proof-only/read-only sanitized descriptor token-validation result-envelope logic with no fixture or dataset addition. FP-0140 ships only docs-and-plan/proof-gate invalid-token challenge implementation planning with no fixture or dataset addition. FP-0141 ships only local invalid-token challenge mapping from sanitized FP-0139 result envelopes with no fixture or dataset addition. These slices add no sample data, source-pack fixtures, eval datasets, package scripts, smoke aliases, screenshots, generated images, public assets, listing copy, generated public prose, app-submission artifacts, source-pack mutations, production token validation runtime, test-double runtime consumption, route consumption of test doubles, token parsing runtime, JWT decoding, token introspection, OAuth/token/session/auth middleware, or FP-0142.
