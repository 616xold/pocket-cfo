...
`plans/FP-0049-board-packet-circulation-note-reset-and-effective-record-hardening.md` is now the active F5C4I contract for explicit `circulationNote` clear-to-absent semantics on that same seam, but benchmark scenarios should not treat that behavior as shipped until the contract lands.
