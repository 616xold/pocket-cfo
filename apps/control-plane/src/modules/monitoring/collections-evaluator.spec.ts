...
describe("collections pressure evaluator", () => {
  it("maps missing receivables-aging source posture to a critical alert", () => {
    const evaluated = evaluateCollectionsPressureMonitor(
      buildCollectionsPosture({ freshnessState: "missing" }),
    );
  ...
  it("maps failed and stale source freshness deterministically", () => {
    const failed = evaluateCollectionsPressureMonitor(
      buildCollectionsPosture({
        failedLatestRun: true,
        freshnessState: "failed",
      }),
    );
    const stale = evaluateCollectionsPressureMonitor(
      buildCollectionsPosture({ freshnessState: "stale" }),
    );
  ...
  it("computes overdue concentration only from source-backed totals", () => {
    ...
    const critical = evaluateCollectionsPressureMonitor(
      buildCollectionsPosture({
        currencyBuckets: [
          buildBucket({
            current: "20.00",
    ...
    const clean = evaluateCollectionsPressureMonitor(
      buildCollectionsPosture({
        currencyBuckets: [
          buildBucket({
            current: "80.00",
...
function buildCollectionsPosture(input: {
  coverageSummary?: FinanceCollectionsPostureView["coverageSummary"];
...
function buildBucket(input: {
  current: string;
  exactBucketTotals?: FinanceCollectionsPostureCurrencyBucket["exactBucketTotals"];
  pastDue: string;
  total: string;
}): FinanceCollectionsPostureCurrencyBucket {
  return {
    currency: "USD",
    totalReceivables: input.total,
    currentBucketTotal: input.current,
    pastDueBucketTotal: input.pastDue,
    exactBucketTotals:
      input.exactBucketTotals ??
      [
        {
          bucketKey: "current",
          bucketClass: "current",
...
function buildSyncRun(input: {
  ...
  return {
    ...
    stats: {},
    errorSummary:
      input.status === "failed" ? "Could not parse receivables-aging rows." : null,
...
function buildFreshness(input: {
  ...
  return {
    ...
    reasonSummary: `Synthetic ${input.state} collections-posture freshness.`,
  };
}
