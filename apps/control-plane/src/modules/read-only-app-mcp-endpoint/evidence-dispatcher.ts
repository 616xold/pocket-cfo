...
export type ReadOnlyEvidenceToolServicePort = {
  ...
  fetchSourceCoverage(): EvidenceToolResponse<unknown>;
...
export class LocalReadOnlyEvidenceToolDispatchAdapter
  implements ReadOnlyEvidenceToolDispatcher
{
  constructor(private readonly evidenceService: ReadOnlyEvidenceToolServicePort) {}
  ...
  dispatchTool(input: {
    ...
    const parsedArguments =
      EVIDENCE_TOOL_DISPATCH_ARGUMENT_SCHEMAS_BY_TOOL[toolName].safeParse(
        input.arguments,
      );
    ...
    content: [
      {
        text:
          refusalReason === null
            ? `Read-only evidence tool ${response.toolName} returned a source-backed structured result.`
            : `Read-only evidence tool ${response.toolName} refused locally: ${refusalReason}.`,
        type: "text",
      },
    ],
    isError: refusalReason !== null,
    structuredContent: {
...
function formatEvidenceDispatchLocalRefusal(input: {
  refusalReason: "invalid_arguments" | "invalid_tool";
  toolName: string;
}): ReadOnlyEvidenceToolDispatchResult {
  return {
    content: [
      {
        text: `Read-only evidence tool ${input.toolName} refused locally: ${input.refusalReason}.`,
        type: "text",
      },
    ],
    isError: true,
    structuredContent: {
      ...
      freshness: {
        ...
        state: "missing",
        summary: "No evidence service dispatch occurred for this failed request.",
      },
      limitations: [
        {
          affectedAnchorIds: [],
...
function refusalReasonFor(response: EvidenceToolResponse<unknown>) {
  ...
  if (hasMissingEvidenceLimitation(response.limitations)) return "missing_evidence";
