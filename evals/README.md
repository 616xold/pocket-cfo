...
- `plans/FP-0049-board-packet-circulation-note-reset-and-effective-record-hardening.md` is now the active F5C4I contract for explicit `circulationNote` clear-to-absent semantics on that same seam, but it is not shipped behavior yet
- grade only the shipped actor-correction truth on the existing seam until FP-0049 lands; do not widen grading into note-reset-as-shipped, multi-packet circulation, actual delivery, PDF export, slide or Marp export, or runtime-codex drafting behavior
