...
`plans/FP-0058-stack-pack-expansion-and-close-control-demo-foundation.md` is now the active implementation-ready F6I contract. The next implementation should extend the existing `pocket-cfo-monitor-demo` stack-pack with normalized close/control checklist expected output and update the existing monitor demo replay proof only. It must not add monitor families, discovery families, routes, schema, migrations, package scripts, eval datasets, runtime-Codex, delivery, reports, approvals, payment behavior, legal or policy advice, collection/customer-contact instructions, or autonomous actions.
