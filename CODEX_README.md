...
FP-0109 is shipped only as a local-only/read-only/dependency-injected evidence tool dispatch adapter for the existing `/mcp` `tools/call` service path. It maps the exact V2G tools to `ReadOnlyEvidenceToolService`, preserves structured evidence/refusal envelopes, keeps default app registration fail-closed unless a dispatcher is explicitly injected, and adds no route path, GET behavior change, DB query, schema/migration, package script, fixture, sample data, source pack, public asset, OAuth/token/session handling, remote MCP deployment, Apps SDK resource, public ChatGPT App behavior, app submission, provider/external/OpenAI/model call, source mutation, finance write, generated finance advice, or autonomous action.
