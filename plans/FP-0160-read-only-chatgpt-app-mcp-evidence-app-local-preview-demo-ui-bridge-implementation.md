...
Same-branch QA found two proof-only compatibility wrinkles before final validation: root-level proof tooling could not import web React packages directly, so the direct FP-0160 proof uses source/static contract checks while the web-focused Vitest specs render the components; and inherited proof scanners needed exact successor compatibility for FP-0160 preview route/spec/doc additions without accepting credential/source-like sentinel strings from test assertions. Both corrections stayed in proof/spec text and did not change runtime behavior.
