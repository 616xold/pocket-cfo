...
- FP-0102 is shipped as `V2V-read-only-chatgpt-app-mcp-endpoint-oauth-remote-mcp-architecture-master-plan-local-v1`, a docs-and-plan plus proof-gate compatibility slice that defines future endpoint/OAuth/token/session/remote-MCP architecture and security-readiness gates only
- FP-0103 is shipped as `V2W-read-only-chatgpt-app-mcp-endpoint-architecture-proof-contracts-foundation-local-v1`, a local/proof-only/read-only contract slice that proves endpoint architecture contracts, endpoint inventory deferral, future endpoint path preconditions, future transport/TLS requirements, request/response envelope posture, refusal/failure behavior, read-only allowlist preservation, proof-source hardening, and no-runtime/no-route/no-endpoint posture only
- FP-0104 is shipped as `V2X-read-only-chatgpt-app-mcp-endpoint-implementation-readiness-and-inventory-master-plan-local-v1`, a docs-and-plan plus proof-gate compatibility slice that names `/mcp` as the only future ChatGPT-facing endpoint path safe to name from current repo truth and official docs, records route ownership, request/response envelope, refusal/failure posture, auth, transport/TLS, logging, deployment deferral, and proof gates only
- FP-0105 is shipped as `V2Y-read-only-chatgpt-app-mcp-endpoint-route-ownership-transport-adapter-proof-contracts-local-v1`, a local/proof-only/read-only contract slice that names `apps/control-plane` Fastify as the documentation-only future `/mcp` route owner family and proves transport-adapter, thin-handler, read-only service/tool dispatch, envelope/refusal, auth-deferral, logging-redaction, deployment-deferral, rollback, no-route, no-runtime, no-OAuth/token/session, no-remote-MCP, no-Apps-SDK-resource, and no-OpenAI-API/model boundaries only
- FP-0106 is shipped as `V2Z-read-only-chatgpt-app-mcp-protocol-envelope-tool-dispatch-proof-contracts-local-v1`, a local/proof-only/read-only contract slice that proves the future `/mcp` protocol envelope, required future MCP/JSON-RPC methods, the official-spec `ping` liveness utility boundary, rejected MCP/JSON-RPC methods, exact V2G read-only tool dispatch, structured evidence/refusal envelopes, argument validation, invalid-tool fail-closed behavior, auth deferral, logging redaction, no-route/no-runtime/no-endpoint posture, no-OpenAI-API/model boundaries, and the pre-FP-0107 successor boundary only
- FP-0107 is shipped as `V2AA-read-only-chatgpt-app-mcp-local-fastify-mcp-route-adapter-foundation-local-v1`, a local-only `apps/control-plane` Fastify `/mcp` protocol-shell slice where `POST /mcp` is the only JSON-RPC request entrypoint, `GET /mcp` is only an SSE-unavailable HTTP 405 boundary with `Allow: POST`, accepted notifications return HTTP 202 with no body, non-local Origin headers fail closed, and the shell handles JSON-RPC envelope validation, `initialize`, `ping`, `notifications/initialized`, `tools/list`, structured fail-closed errors, and fail-closed `tools/call` only
- FP-0108 is shipped as `V2AB-read-only-chatgpt-app-mcp-read-only-evidence-tool-dispatch-contracts-local-v1`, a local/proof-only/read-only contract slice that proves exact V2G evidence tool dispatch mappings, strict argument schemas, future read-only evidence/source-authority dependency lanes, structured evidence/refusal envelopes, freshness/source-anchor requirements, no raw dump, no generated finance advice, no source mutation, no finance write, no provider/external/OpenAI/model calls, no dispatch runtime, and no route behavior change while preserving the FP-0107 fail-closed route shell only
- FP-0109 is shipped as `V2AC-read-only-chatgpt-app-mcp-read-only-evidence-tool-dispatch-adapter-implementation-local-v1`, a local-only/read-only dependency-injected implementation slice that maps the exact V2G tools to `ReadOnlyEvidenceToolService` through the existing `/mcp` `tools/call` service path, enforces expected `companyKey`, honors declared arguments or fails closed including source-coverage `sourceId` and unsupported `periodKey`, mirrors `structuredContent` with bounded JSON text, and keeps default route registration fail-closed unless a dispatcher is explicitly injected
- FP-0110 is shipped as `V2AD-read-only-chatgpt-app-mcp-default-local-evidence-dispatch-enablement-master-plan-local-v1`, a docs-and-plan plus proof-gate compatibility slice that plans future default local evidence dispatch enablement only. It requires a later implementation plan to prove explicit app construction, one expected `companyKey`, existing local read-only evidence artifact lanes, no-real-finance-data/no-public-demo-data posture, and default fail-closed behavior before runtime wiring.
- FP-0111 is shipped as `V2AE-read-only-chatgpt-app-mcp-default-local-evidence-dispatch-wiring-local-v1`, a local-only/read-only explicit app-construction wiring slice that passes a supplied read-only MCP endpoint service into the existing `/mcp` route while default `buildApp()` remains fail-closed without that dependency.
- FP-0112 is shipped as `V2AF-read-only-chatgpt-app-mcp-remote-public-deployment-oauth-readiness-master-plan-local-v1`, a docs-and-plan plus proof-gate compatibility slice that plans remote/public MCP deployment readiness, OAuth/token/session prerequisites, company/user/org binding, host/CORS/CSP/TLS/rate-limit/log-redaction/rollback gates, no-real-finance-data posture, and app-submission blockers without implementing them.
- FP-0113 is shipped as `V2AG-read-only-chatgpt-app-mcp-oauth-token-session-security-contracts-foundation-local-v1`, a local/proof-only/read-only contract slice that proves OAuth deferral, token/session deferral, auth middleware deferral, public exposure block, authenticated user/org/company binding requirements, client `companyKey` selector fail-closed posture, scope minimization, audience validation, token passthrough prohibition, token failure modes, refresh/offline-access review, token storage/redaction/revocation/rotation boundaries, no-token-leakage, no-real-finance-data, no-route-change, no-remote-MCP, no-Apps-SDK-resource, no-app-submission, no-DB, no-schema/migration, no-public-asset, no-OpenAI-API/model/key, no-provider/external-call, no-source-mutation, and no-finance-write boundaries only.
- FP-0114 is shipped as `V2AH-read-only-chatgpt-app-mcp-remote-host-readiness-security-contracts-foundation-local-v1`, a local/proof-only/read-only contract slice that proves remote deployment deferral, current local `/mcp` non-exposability, stable HTTPS/TLS and canonical MCP resource URI prerequisites, `/mcp` as the only future public MCP endpoint path, Streamable HTTP compatibility, GET SSE deferral, Origin/CORS/CSP requirements, rate-limit/abuse controls, logging redaction, observability/audit correlation, rollback/incident response, health/readiness deferral, no-real-finance-data/no-public-demo-data posture, FP-0113 OAuth/security prerequisites, no remote runtime, no-route-change, no-new-route-path, no-deployment-config, no-OAuth/token/session/auth-middleware implementation, no-Apps-SDK-resource, no-app-submission, no-DB, no-schema/migration, no-package-script, no-public-asset, no-OpenAI-API/model/key, no-provider/external-call, no-source-mutation, no-finance-write, and docs-only FP-0115 successor boundaries only.
- FP-0115 is shipped as `V2AI-read-only-chatgpt-app-mcp-remote-host-implementation-sequencing-master-plan-local-v1`, a docs-and-plan plus proof-gate compatibility slice that plans remote MCP host implementation sequencing, provider/host candidate analysis, canonical resource URI prerequisites, public `/mcp` gates, OAuth/token/session/auth middleware prerequisites, HTTPS/TLS, Origin/CORS/CSP, rate-limit/abuse, logging redaction, observability/audit, rollback, no-real-finance-data posture, and public app submission blockers without implementing them.
- FP-0116 is shipped as `V2AJ-read-only-chatgpt-app-mcp-remote-host-resource-contracts-local-v1`, a local/proof-only/read-only contract slice that proves host owner remains unresolved with implementation blocked, current local `/mcp` remains local-only, provider remains neutral, future separate MCP server package or gateway/wrapper is preferred unless later proof decides otherwise, exact stable HTTPS canonical resource URI and protected-resource metadata are prerequisites, public `/mcp` remains the only future public MCP endpoint path, workspace/tenant template URLs are rejected, local tunnels are development-only, and FP-0114/FP-0115 proof durability is tightened.
- FP-0117 is shipped as `V2AK-read-only-chatgpt-app-mcp-oauth-token-session-auth-implementation-sequencing-master-plan-local-v1`, a docs-and-plan plus proof-gate compatibility slice that plans OAuth/token/session/auth implementation sequencing and protected-resource metadata readiness only. It decides OAuth/token/session/auth implementation cannot start from current repo truth, protected-resource metadata and WWW-Authenticate `resource_metadata` readiness should lead the next auth-adjacent lane, token validation must wait for a canonical public resource URI, provider/auth-server selection remains unresolved/provider-neutral, token/session storage and auth middleware must wait for fail-closed proof gates, authenticated company binding gates `companyKey`, and public app submission remains future-only.
- FP-0118 is shipped as `V2AL-read-only-chatgpt-app-mcp-protected-resource-metadata-auth-challenge-readiness-contracts-local-v1`, a local/proof-only/read-only contract slice that proves protected-resource metadata document shape, canonical resource URI dependency, authorization server requirements, read-only least-privilege scope posture, bearer method posture, WWW-Authenticate `resource_metadata` challenge readiness, metadata discovery, scope challenge readiness, token failure challenge posture, no-token-leakage, route deferral, WWW-Authenticate route deferral, and no-runtime posture without implementing routes or runtime auth.
- FP-0119 is shipped as `V2AM-read-only-chatgpt-app-mcp-protected-resource-metadata-route-implementation-sequencing-master-plan-local-v1`, a docs-and-plan plus proof-gate compatibility slice that plans protected-resource metadata route implementation sequencing and WWW-Authenticate `resource_metadata` challenge sequencing only. It decides route implementation cannot start from current repo truth until canonical public resource URI, route tests, metadata document tests, no-token-leakage gates, and local `/mcp` unchanged-behavior gates are green, and keeps WWW-Authenticate behavior in a later/separate lane.
- FP-0120 is shipped as `V2AN-read-only-chatgpt-app-mcp-canonical-resource-auth-server-readiness-contracts-local-v1`, a local/proof-only/read-only contract slice for canonical public MCP resource URI requirements, deferred exact stable HTTPS URI decision, no selector/query/fragment/local-tunnel authority, provider-neutral `authorization_servers` readiness, RFC 9728 route-path derivation, WWW-Authenticate `resource_metadata` URL readiness, durable known-safe route inventory proofing, validation-gated metadata URL derivation, and no-route-runtime posture.
- FP-0121 is shipped as `V2AO-read-only-chatgpt-app-mcp-protected-resource-metadata-route-implementation-planning-local-v1`, a docs-and-plan/proof-gate-only readiness slice for future protected-resource metadata route implementation.
- FP-0122 is shipped as `V2AP-read-only-chatgpt-app-mcp-protected-resource-metadata-document-builder-contracts-local-v1` with post-merge credential/userinfo hardening, a local/proof-only/read-only protected-resource metadata document-builder and deferred route-response contract slice. It keeps route implementation blocked until canonical URI, `authorization_servers`, exact route path, route tests, metadata document tests, no-token-leakage proof, `/mcp` unchanged-behavior proof, authenticated company binding, and FP-0122 builder contract gates are green in a later route plan.
- FP-0123 is shipped as `V2AQ-read-only-chatgpt-app-mcp-protected-resource-metadata-route-input-evidence-contracts-local-v1`, a local/proof-only/read-only protected-resource metadata route-input evidence bundle and route-path decision contract slice. It keeps route implementation blocked until the route-input bundle, route path decision, no-token-leakage, authenticated company binding, `/mcp` unchanged behavior, FP-0122 builder, FP-0121 planning, FP-0120 canonical URI/auth-server, and prior boundary gates are green in a later route implementation plan.
- FP-0124 is shipped as `V2AR-read-only-chatgpt-app-mcp-protected-resource-metadata-route-implementation-master-plan-local-v1`, a docs-and-plan plus proof-gate compatibility route implementation planning slice. It kept actual route implementation blocked until a future narrow plan proved explicit FP-0123 route-input evidence dependency injection, fail-closed canonical URI/auth-server/builder-output/no-token-leakage/company-binding/`/mcp` unchanged gates, `/mcp`-derived route-path posture, route tests, and deferred WWW-Authenticate separation.
- FP-0125 is shipped as `V2AS-read-only-chatgpt-app-mcp-protected-resource-metadata-local-route-implementation-local-v1`, a local-only/read-only explicit-dependency route slice. It registers exactly GET `/.well-known/oauth-protected-resource/mcp` only when app construction supplies a valid FP-0123 route-input evidence bundle, keeps default `buildApp()` route-absent, emits only bounded FP-0122 metadata fields, and preserves `/mcp` behavior.
- FP-0126 is shipped as `V2AT-read-only-chatgpt-app-mcp-www-authenticate-auth-challenge-sequencing-master-plan-local-v1`, a docs-and-plan plus proof-gate compatibility slice for future WWW-Authenticate `resource_metadata` auth-challenge behavior sequencing. It preserves FP-0125 protected-resource metadata local route behavior and evidence-coherence hardening, keeps `/mcp` behavior unchanged, keeps token failure statuses in a later token-validation lane, and adds no runtime challenge, OAuth/token/session/auth middleware, remote/public/App SDK/app-submission, DB/schema/package/source/finance-write, or autonomous-action scope.
- FP-0127 is shipped as `V2AU-read-only-chatgpt-app-mcp-www-authenticate-auth-challenge-contracts-foundation-local-v1`, a local/proof-only/read-only contract slice for future WWW-Authenticate `resource_metadata` auth-challenge behavior. It proves Bearer challenge shape, local-vs-public metadata reference posture, missing/invalid token challenge posture, future-only token failure modes, read-only least-privilege scope challenge guidance, no-token-leakage, unchanged `/mcp`, and no-runtime posture without adding route behavior, header emission, protected-resource metadata route behavior changes, OAuth/token/session/auth middleware, token validation runtime, remote/public/App SDK/app-submission, DB/schema/package/source/finance-write, or autonomous-action scope.
- FP-0128 is shipped as `V2AV-read-only-chatgpt-app-mcp-token-validation-failure-readiness-contracts-local-v1`, a local/proof-only/read-only contract slice for future token-validation failure-mode and auth-challenge readiness. It proves token-validation runtime deferral, token parsing deferral, token/session storage deferral, auth middleware deferral, token failure taxonomy, audience/resource validation prerequisites, read-only scope challenge prerequisites, authenticated user/org/company binding prerequisites, client `companyKey` selector-only posture, token passthrough prohibition, no-token-leakage, no-runtime posture, and FP-0127 challenge contract preservation without adding route behavior, header emission, protected-resource metadata route behavior changes, OAuth/token/session/auth middleware, token validation runtime, token parsing runtime, remote/public/App SDK/app-submission, DB/schema/package/source/finance-write, or autonomous-action scope.
- FP-0129 is shipped as `V2AW-read-only-chatgpt-app-mcp-www-authenticate-challenge-implementation-sequencing-master-plan-local-v1`, a docs-and-plan/proof-gate-only sequencing slice for future WWW-Authenticate challenge implementation. It decides missing-token may be the first narrow future implementation candidate, invalid-token cannot become semantic token validation before token-validation runtime, later failure statuses remain in the token-validation runtime lane, public challenge references wait for canonical public URL and remote/public host proof, JSON-RPC refusals stay separate from auth challenge emission, and protected-resource metadata route behavior remains unchanged.
- FP-0130 is shipped as `V2AX-read-only-chatgpt-app-mcp-www-authenticate-missing-token-challenge-local-implementation-v1`, a local-only/read-only explicit-dependency missing-token challenge slice for existing `POST /mcp`. Default `buildApp()` and default `/mcp` behavior remain unchanged; only explicit app construction input that co-supplies accepted protected-resource metadata route-input evidence can emit the bounded `401` Bearer `resource_metadata` challenge, challenge-only construction fails closed before route registration, and Authorization-present requests fail closed without token parsing or validation.
- FP-0131 shipped as `V2AY-read-only-chatgpt-app-mcp-token-validation-invalid-token-runtime-sequencing-master-plan-local-v1`, a docs-and-plan/proof-gate compatibility slice for token-validation and invalid-token runtime sequencing. FP-0132 shipped as the V2AZ local/proof-only/read-only token-validation runtime contract foundation. FP-0133 shipped as the V2BA local/proof-only/read-only token-validation test-double contract foundation. FP-0134 is the V2BB local-only/read-only synthetic token-validation test-double evaluator. FP-0135 is the V2BC docs-and-plan/proof-gate compatibility invalid-token WWW-Authenticate challenge sequencing plan. FP-0136 is the V2BD local/proof-only/read-only invalid-token challenge contract foundation. FP-0137 is the V2BE docs-and-plan/proof-gate invalid-token challenge implementation-readiness master plan. FP-0138 is the V2BF docs-and-plan/proof-gate token-validation runtime implementation-planning master plan. FP-0139 is the V2BG local/proof-only/read-only token-validation result-envelope implementation slice. FP-0140 is the V2BH docs-and-plan/proof-gate invalid-token challenge implementation-planning slice. FP-0141 is the V2BI local-only/read-only invalid-token challenge local runtime slice consuming sanitized FP-0139 result envelopes only. FP-0142 is the V2BJ docs-and-plan/proof-gate invalid-token route integration sequencing slice for Authorization-present activation, missing-token precedence, protected-resource metadata separation, FP-0139 result-envelope-only source posture, app-construction co-registration prerequisites, and failure taxonomy / HTTP / WWW-Authenticate consistency gates. FP-0143 is the V2BK local-only/read-only explicit app-construction wiring slice that forwards an optional sanitized FP-0139 invalid-token result-envelope dependency from `buildApp({ container })` into existing `/mcp` route registration. FP-0144 is the V2BL docs-and-plan/proof-gate production token-validation sequencing plan that keeps JWT/JWKS versus opaque-token introspection unresolved/provider-neutral, defers Authorization parsing, maps future failure states to FP-0139 envelopes, keeps missing-token behavior separate, and requires one more proof-hardening/runtime-contract slice. FP-0145 is the V2BM local proof-hardening/runtime-contract slice that records provider-neutral runtime contracts, sanitized no-raw-token input boundaries, validation-source contracts, prerequisite gates, FP-0139 envelope mapping, and FP-0144 top-level forbidden-source-scope proof hardening. FP-0146 is the V2BN parser-contract/provider-selection-proof slice that records sanitized Authorization parser output fields, FP-0139/FP-0130 parser failure mapping, provider-selection criteria, and provider-neutral/deferred posture. FP-0147 is the V2BO provider-selection evidence-hardening/canonical resource OAuth-readiness proof slice that records canonical resource URI, protected-resource metadata, resource indicator, authorization-server discovery, provider evidence matrix, issuer/audience/resource, scope/RBAC/org/company, no-token-passthrough, no-credential-forwarding, SSRF/URL-safety, replay, revocation, service-unavailable, dev/test tenant, and mTLS/egress/IP allowlist future evidence requirements. FP-0148 is the V2BP parser implementation-readiness/no-token-retention proof-hardening slice that records future parser input/output boundaries, no raw header/token retention, no token-derived fingerprint fields, FP-0139/FP-0130 failure mapping, and shared proof-only no-token-leakage fixture sanitization. FP-0149 is the V2BQ pure-domain/local-only Authorization parser implementation slice that classifies sanitized presence/scheme posture and returns only FP-0146 output fields. FP-0150 is the V2BR pure-domain parser material-observation hardening and route-integration sequencing slice. FP-0151 is the V2BS route-integration implementation-readiness slice that records route-safe parser decision contracts, future explicit dependency injection, missing-token precedence, FP-0139 envelope mapping, invalid-token downstream posture, no-token-leakage posture, and protected-resource metadata preservation. FP-0152 is the V2BT explicit-dependency route-integration implementation slice that adds only optional route-safe parser decision injection to the local `/mcp` route. FP-0153 is the V2BU explicit app-construction parser route-decision pass-through and co-registration hardening slice that adds only an optional `AppContainer` port, explicit `buildApp({ container })` pass-through, and parser-dependency registration hardening against missing invalid-token challenge co-registration. FP-0154 is the V2BV local adapter construction-readiness/proof slice that records future explicit local adapter input/output/composition/failure-mapping/test-matrix requirements without implementing or constructing the adapter. FP-0155 is the V2BW explicit pure-domain/local-only adapter implementation/factory slice. FP-0156 is the V2BX runtime-import isolation and explicit opt-in app-construction adapter injection slice. FP-0157 is the V2BY local-only/read-only auth demo/smoke harness and helper double-injection hardening slice. FP-0158 is the V2BZ local-only/read-only Evidence App demo bridge slice with separate explicit-helper auth and synthetic/stub evidence dispatch lanes. These slices keep test-double runtime consumption, route consumption of test doubles, production token-validation runtime, default parser wiring, parser construction inside routes, default adapter wiring, default evidence dispatch wiring, token parsing, JWT decoding, JWKS fetching/caching, token introspection, OAuth/token/session/auth middleware, remote/public/App SDK/app-submission, DB/schema/package/source/finance-write, public asset, provider/OpenAI-call, autonomous-action, and FP-0159 scope blocked.
- no public app implementation, MCP server runtime, route implementation beyond the FP-0125 local metadata route, FP-0107 local `/mcp` shell, FP-0130 explicit missing-token seam, FP-0141 explicit invalid-token sanitized-envelope seam, FP-0142 docs/proof route integration sequencing boundary, FP-0143 explicit app-construction invalid-token wiring, FP-0144 docs/proof production token-validation sequencing boundary, FP-0145 runtime-contract/proof-hardening boundary, FP-0146 parser-contract/provider-selection-proof boundary, FP-0147 provider-selection evidence-hardening boundary, FP-0148 parser implementation-readiness/proof-hardening boundary, FP-0149 parser pure-domain implementation boundary, FP-0150 parser material-observation/route-integration sequencing boundary, FP-0151 route-integration implementation-readiness boundary, FP-0152 explicit-dependency route-integration boundary, FP-0153 explicit app-construction parser route-decision pass-through/co-registration hardening boundary, FP-0154 local adapter construction-readiness/proof boundary, FP-0155 explicit local adapter implementation/factory boundary, FP-0156 explicit app-construction adapter injection boundary, FP-0157 local auth demo/smoke harness boundary, FP-0158 local Evidence App demo bridge boundary, FP-0134 local synthetic evaluator, FP-0135 docs-only sequencing boundary, FP-0136 local/proof-only invalid-token contract boundary, FP-0137 docs/proof implementation-readiness boundary, FP-0138 docs/proof runtime implementation-planning boundary, FP-0139 local/proof-only result-envelope boundary, FP-0140 docs/proof implementation-planning boundary, and FP-0094/FP-0096/FP-0097 existing local preview route boundary, WWW-Authenticate behavior beyond FP-0130/FP-0141/FP-0143 plus FP-0152 existing sanitized challenge routing, endpoint expansion, hidden evidence service construction, Apps SDK iframe/resource registration, OAuth implementation, token/session implementation, production token validation runtime, default parser wiring, parser construction inside routes, default adapter wiring, default evidence dispatch wiring, local demo or preview UI wiring beyond FP-0158, token parsing runtime, test-double runtime consumption, route consumption of test doubles, JWT decoding, auth middleware implementation, app submission, schema, migration, package script, smoke alias, eval dataset, fixture, sample data, public demo data, source-pack behavior, OpenAI API/model call, hosted tool, vector/file-search integration, OCR, PageIndex, provider setup, certification, delivery, deployment, external communications, source mutation, finance write, generated product prose, runtime-Codex finance output, autonomous action, screenshot, generated image, public asset, listing copy, app-submission artifact, FP-0159, or default/public evidence dispatch beyond FP-0111 explicit local wiring starts from FP-0087 through FP-0158 or roadmap text alone
- optional ChatGPT App/MCP implementation beyond FP-0158, route expansion beyond FP-0125/FP-0107/FP-0130/FP-0141/FP-0143/FP-0152/FP-0153/FP-0156/FP-0157/FP-0158, protected-resource metadata route implementation beyond the FP-0125 exact local path, production token-validation semantic challenge behavior, endpoint/OAuth/remote MCP implementation, auth middleware implementation, production token validation runtime, local demo or preview UI wiring beyond the FP-0158 local smoke bridge, token parsing runtime, test-double runtime consumption, route consumption of test doubles, Apps SDK resource implementation, app submission, iOS, OpenClaw, deployment, default/public evidence dispatch beyond FP-0111 explicit local wiring, or other distribution tracks require later named Finance Plans after V2 foundations and shipped FP-0158 gates are proven
...
- FP-0102 expansion beyond docs-only endpoint/OAuth/remote-MCP architecture and security-readiness planning
- FP-0103 expansion beyond local/proof-only/read-only endpoint architecture proof contracts
- FP-0104 expansion beyond docs-only endpoint implementation readiness and exact future endpoint inventory planning
- FP-0105 expansion beyond local/proof-only/read-only endpoint route ownership and transport-adapter proof contracts
- FP-0106 expansion beyond local/proof-only/read-only MCP protocol envelope, `ping` liveness, and tool-dispatch proof contracts
- FP-0107 expansion beyond local-only `/mcp` protocol-shell hardening and fail-closed `tools/call`
- FP-0108 expansion beyond local/proof-only/read-only evidence tool dispatch contracts
- FP-0109 expansion beyond explicit dependency-injected local read-only evidence dispatch
- FP-0110 expansion beyond docs-and-plan/proof-gate compatibility for default local dispatch enablement
- FP-0111 expansion beyond explicit local app-construction wiring
- FP-0112 expansion beyond docs-and-plan/proof-gate compatibility for remote/public MCP OAuth readiness
- FP-0113 expansion beyond local/proof-only OAuth/token/session/auth middleware and public MCP security contracts
- FP-0114 expansion beyond local/proof-only remote MCP host readiness and public transport/security contracts
- FP-0115 expansion beyond docs-only remote MCP host implementation sequencing
- FP-0116 expansion beyond local/proof-only remote host resource contracts
- FP-0117 expansion beyond docs-and-plan/proof-gate compatibility for OAuth/token/session/auth implementation sequencing
- FP-0119 expansion beyond docs-and-plan/proof-gate compatibility for protected-resource metadata route sequencing
- FP-0120 expansion beyond local/proof-only/read-only canonical resource/auth-server readiness contracts
