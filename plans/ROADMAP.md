...
- FP-0109 is shipped as `V2AC-read-only-chatgpt-app-mcp-read-only-evidence-tool-dispatch-adapter-implementation-local-v1`, a local-only/read-only dependency-injected implementation slice that maps the exact V2G tools to `ReadOnlyEvidenceToolService` through the existing `/mcp` `tools/call` service path while keeping default route registration fail-closed unless a dispatcher is explicitly injected
