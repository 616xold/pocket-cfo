...
- FP-0131 shipped as `V2AY-read-only-chatgpt-app-mcp-token-validation-invalid-token-runtime-sequencing-master-plan-local-v1`, a docs-and-plan/proof-gate compatibility slice for token-validation and invalid-token runtime sequencing. FP-0132 shipped as the V2AZ local/proof-only/read-only token-validation runtime contract foundation. FP-0133 shipped as the V2BA local/proof-only/read-only token-validation test-double contract foundation. FP-0134 is the V2BB local-only/read-only synthetic token-validation test-double evaluator. It keeps test-double runtime consumption, token-validation runtime, token parsing, JWT decoding, invalid-token challenge behavior, OAuth/token/session/auth middleware, remote/public/App SDK/app-submission, DB/schema/package/source/finance-write, public asset, provider/OpenAI-call, autonomous-action, and FP-0135 scope blocked.
- no public app implementation, MCP server runtime, route implementation beyond the FP-0125 local metadata route, FP-0107 local `/mcp` shell, FP-0130 explicit missing-token seam, FP-0134 local synthetic evaluator, and FP-0094/FP-0096/FP-0097 existing local preview route boundary, WWW-Authenticate behavior beyond FP-0130, invalid-token runtime behavior, endpoint expansion, hidden evidence service construction, Apps SDK iframe/resource registration, OAuth implementation, token/session implementation, token validation runtime, token parsing runtime, test-double runtime consumption, JWT decoding, auth middleware implementation, app submission, schema, migration, package script, smoke alias, eval dataset, fixture, sample data, public demo data, source-pack behavior, OpenAI API/model call, hosted tool, vector/file-search integration, OCR, PageIndex, provider setup, certification, delivery, deployment, external communications, source mutation, finance write, generated product prose, runtime-Codex finance output, autonomous action, screenshot, generated image, public asset, listing copy, app-submission artifact, FP-0135, or default/public evidence dispatch beyond FP-0111 explicit local wiring starts from FP-0087 through FP-0134 or roadmap text alone
- optional ChatGPT App/MCP implementation beyond FP-0134, route expansion beyond FP-0125/FP-0107/FP-0130, protected-resource metadata route implementation beyond the FP-0125 exact local path, invalid-token semantic challenge behavior, endpoint/OAuth/remote MCP implementation, auth middleware implementation, token validation runtime, token parsing runtime, test-double runtime consumption, Apps SDK resource implementation, app submission, iOS, OpenClaw, deployment, default/public evidence dispatch beyond FP-0111 explicit local wiring, or other distribution tracks require later named Finance Plans after V2 foundations and shipped FP-0134 gates are proven
