...
This is the active Finance Plan contract for the Pocket CFO F6Y planning slice.
The target phase is `F6`, and the implementation slice is exactly `F6Y-board-lender-document-source-pack-foundation`.

The user-visible goal is narrow: after shipped F6A through F6W, plan one additional checked-in document source-pack family for board and lender documents using existing source registry and CFO Wiki bind, compile, and read routes only.
F6Y is allowed to plan source-pack proof posture.
It is not allowed to implement product runtime behavior in this docs-and-plan slice.

Repo truth supports this reduced F6Y shape because the shipped F6L, F6O, F6R, F6U, and F6W source-pack proofs are green; the shipped F6T certification-safety posture is green and non-certifying; the CFO Wiki source binding, compile, source-list, source-digest, source-coverage, index/log, and page-read surfaces are green; and `board_material` plus `lender_document` are already domain-supported CFO Wiki document roles on the generic document bind/extract/source-digest path.
The next implementation must preserve that posture rather than widening it.
...
F6Y should close the proof gap by requiring a deterministic source-pack proof that covers both roles, rather than by adding routes or schema.
...
A future F6Y proof may seed a deterministic demo finance company row as proof setup only if needed by that existing route; this would not be product runtime behavior, a new route, or source-pack behavior beyond setup.
...
Rationale: the expected posture should include source file list, source roles/descriptors, document role, source kind, media type, expected CFO Wiki document extract posture, expected source digest posture, expected source coverage posture, source-list posture, freshness posture, limitations, absence boundaries, raw fixture immutability, and fixed monitor/discovery family boundaries. It should add no new monitor result semantics, checklist item families, readiness behavior, acknowledgement behavior, delivery-readiness behavior, review-summary behavior, provider-boundary behavior, certification-boundary behavior, human-confirmation behavior, certification-safety behavior, investigation behavior, reporting behavior, legal advice, or policy advice.
...
Pocket CFO has shipped F6A through F6W.
FP-0050 through FP-0071 are shipped records.
This FP-0072 file is the active implementation-ready planning contract for the next F6Y source-pack slice.
...
This docs-and-plan slice does not implement F6Y.
The later implementation should start from this contract only after the active branch is clean and the readiness gate still passes.

First, add one board/lender document source-pack manifest contract beside the shipped source-pack manifests.
The source-pack family should be named `pocket-cfo-board-lender-document-source-pack`.
It should include exactly two document source roles: `board_material` and `lender_document`.
It should include source kind `document`, document roles `board_material` and `lender_document`, media types `text/markdown` and `text/plain`, and expected document kinds `markdown_text` and `plain_text`.
...
Second, add one checked-in immutable fixture directory for F6Y.
The fixture should contain static markdown and/or plain-text board and lender document source files plus one normalized expected source/wiki posture file.
The fixture content may describe finance document posture only as source-pack evidence, not as board advice, lender advice, legal advice, policy advice, report prose, certification language, approval language, or delivery text.
The expected posture should avoid generated IDs, timestamps, storage refs, or any other volatile fields.

Third, add one direct deterministic proof tool.
...
It should verify source-list, source-digest, source-coverage, index/log, and page-read posture.
It may read shipped reporting, close/control, delivery-readiness, provider-boundary, certification-boundary, human-confirmation, and certification-safety surfaces as validation context only.
It must not create reports, approvals, delivery records, provider records, certification records, missions, monitor results, generated prose, runtime-Codex threads, or finance actions.
...
Fifth, active docs should be refreshed after implementation exists and the direct proof passes.
The implementation should not add a package script, root smoke alias, eval dataset, route, schema, migration, UI, provider integration, credential scaffold, report release, report circulation, approval workflow, or product runtime behavior unless this plan is explicitly amended before code changes.
...
The F6Y implementation validation must run at least the same ladder, plus the future direct proof command after it exists.
...
- `plans/FP-0072-board-lender-document-source-pack-foundation.md` is the active F6Y implementation-ready contract
- `plans/FP-0071-policy-covenant-document-source-pack-foundation.md` remains the shipped F6W record
- active docs point future Codex threads at FP-0072 as active planning truth and FP-0071 as shipped F6W truth
- FP-0050 through FP-0071 remain shipped F6A through F6W records
- no FP-0073 is created
- F6Y is planned as one board/lender document source-pack foundation only
- F6V, F6X, F6Z, and later plans are named only as likely later slices and are not created

## Idempotence and Recovery

This future implementation slice should be retry-safe because it will use static fixture files, object-store source uploads, CFO Wiki source bindings, deterministic compile/read routes, and normalized expected-output comparison.
...
Rollback for the future implementation should be simple: remove only the F6Y manifest, fixture family, proof tool, focused tests, and docs changes from the implementation branch.
...
This docs-and-plan slice creates or updates only:

- `plans/FP-0072-board-lender-document-source-pack-foundation.md`
- `plans/FP-0071-policy-covenant-document-source-pack-foundation.md` for the tiny F6W proof-setup truthfulness polish
- `README.md`
- `START_HERE.md`
- `docs/ACTIVE_DOCS.md`
- `plans/ROADMAP.md`
...
This docs-and-plan slice must not add code, routes, schema, migrations, package scripts, smoke commands, eval datasets, fixtures, implementation scaffolding, provider integrations, credential scaffolding, outbox send behavior, UI, approval workflow, report-release behavior, certification behavior, or product runtime behavior.

Do not create FP-0073.
Do not start F6Y implementation in this slice.
...
This docs-and-plan slice establishes F6Y as implementation-ready only because repo truth supports a safe board/lender document source-pack foundation through existing source and CFO Wiki routes.
F6Y remains one deterministic source-pack planning slice and keeps provider integration, actual certification, external delivery, approvals, reports, runtime-Codex, monitor families, discovery families, generated prose, and autonomous actions out of scope.

Recommendation: after this docs-and-plan PR is reviewed and merged, start F6Y implementation next from this FP-0072 contract if the validation ladder remains green. Do not start F6V provider integration, F6X actual certification, F6Z exit audit, or later work before F6Y is either implemented and closed out or a new plan names a concrete blocker.
