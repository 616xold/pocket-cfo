...
`plans/FP-0067-contract-obligation-source-pack-foundation.md` is now the active implementation-ready F6R contract.
F6R is narrowed to one future contract/obligation source-pack foundation only, with source role limited to `contract_metadata`, extractor key limited to `contract_metadata_csv`, proof surface limited to existing source registry and Finance Twin contract/obligation routes, no product runtime behavior, no package script or smoke alias, no monitor or discovery family, and no F6R implementation started in this docs-and-plan slice.
