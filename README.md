...
`plans/FP-0060-close-control-acknowledgement-foundation.md` is the active F6K implementation-ready contract. F6K is planned only as internal close/control acknowledgement readiness over shipped checklist/readiness posture; no F6K implementation has started, and the plan forbids approvals, close-complete status, delivery, notifications, reports, runtime-Codex, mission creation, monitor reruns, source mutation, finance actions, new monitor families, and new discovery families.
