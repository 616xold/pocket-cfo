...
`plans/FP-0062-external-notification-delivery-planning-foundation.md` is now the active F6M docs-and-plan contract. It is implementation-ready only for deterministic internal delivery-readiness from shipped F6J operator-readiness and F6K acknowledgement-readiness posture; it does not implement external delivery, notification providers, outbox sends, scheduled notifications, auto-send, approvals, reports, runtime-Codex drafting, generated notification prose, mission creation, monitor reruns, source mutation, finance writes, or autonomous action.
