...
- FP-0065 is the active F6P implementation-ready contract for one deterministic internal external-provider-boundary/readiness foundation only, derived from shipped F6M/F6N posture, with no provider integration, provider credential flow, delivery, outbox send, report, approval, generated prose, source mutation, runtime-Codex, finance action, new monitor family, or new discovery family
...
`plans/FP-0065-external-provider-boundary-foundation.md` is now the active F6P implementation-ready contract.
The first F6P implementation should start next only if it stays provider-boundary/readiness-only: no actual external delivery, provider calls, provider credentials, outbox sends, reports, approvals, generated prose, source mutation, runtime-Codex, finance actions, new monitor families, or new discovery families.
