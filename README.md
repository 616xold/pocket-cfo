...
The active implementation-ready contract is now `plans/FP-0064-receivables-payables-source-pack-foundation.md`.
It plans F6O as one receivables/payables source-pack foundation only, with source roles limited to `receivables_aging` and `payables_aging`, extractor keys limited to `receivables_aging_csv` and `payables_aging_csv`, no new monitor or discovery families, and no product runtime behavior.
...
`plans/FP-0064-receivables-payables-source-pack-foundation.md` is now the active F6O implementation-ready contract. F6O is narrowed to one deterministic receivables/payables source-pack planning slice with no code, fixtures, routes, schema, package scripts, smoke aliases, eval datasets, product runtime behavior, delivery, reports, approvals, monitor-family expansion, discovery-family expansion, or implementation work until a future implementation thread follows that plan.
