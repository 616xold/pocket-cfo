...
The active next contract is `plans/FP-0072-board-lender-document-source-pack-foundation.md`.
It plans F6Y as one future board/lender document source-pack foundation only, limited to existing `board_material` and `lender_document` document roles, source kind `document`, deterministic markdown/plain-text paths, existing source registry and CFO Wiki bind/compile/read routes, no product runtime behavior, no FP-0073, and no F6V/F6X/F6Z implementation.
