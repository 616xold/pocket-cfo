...
`plans/FP-0061-source-pack-expansion-foundation.md` is now the active implementation-ready F6L contract. It narrows F6L to one future `bank-card-source-pack-foundation` implementation slice over checked-in bank-account-summary and card-expense source-pack posture only. No F6L implementation has started; do not add code, fixtures, routes, schema, package scripts, smokes, eval datasets, runtime-Codex, delivery, reports, approvals, monitor families, or discovery families outside FP-0061.
