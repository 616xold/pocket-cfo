...
`plans/FP-0066-close-control-certification-boundary-foundation.md` is the active F6Q implementation-ready planning contract.
F6Q may start next only as a deterministic internal close/control certification-boundary/readiness foundation over shipped F6N/F6P posture, with no actual certification, close-complete status, sign-off, attestation, legal opinion, audit opinion, approval, report release, report circulation, external delivery, provider call, outbox send, generated prose, runtime-Codex, source mutation, finance action, new monitor family, or new discovery family.
No F6Q implementation has started.
