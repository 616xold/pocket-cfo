...
`plans/FP-0069-ledger-reconciliation-source-pack-foundation.md` is now the active implementation-ready F6U contract.
F6U is planned as one ledger/reconciliation source-pack foundation only, with future source roles limited to `chart_of_accounts`, `trial_balance`, and `general_ledger`, future extractor keys limited to `chart_of_accounts_csv`, `trial_balance_csv`, and `general_ledger_csv`, proof surface limited to existing source registry and Finance Twin sync/read/reconciliation routes, no product runtime behavior, no package script or smoke alias unless FP-0069 is explicitly amended, no new monitor or discovery families, and no delivery, provider, approval, report, certification, runtime-Codex, generated prose, finance action, source mutation outside proof upload/sync setup, or autonomous action.
