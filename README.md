...
`plans/FP-0070-close-control-certification-safety-foundation.md` is now the active F6T implementation-ready planning contract.
F6T is limited to a future deterministic internal close/control certification-safety/readiness foundation only; it is explicitly not actual certification, close complete, sign-off, attestation, assurance, legal opinion, audit opinion, approval, report release, report circulation, external delivery, provider integration, outbox send behavior, runtime-Codex, generated prose, source mutation, finance write, advice/instruction, or autonomous action.
