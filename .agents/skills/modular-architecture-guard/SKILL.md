...
description: Use when implementing or refactoring code. Enforces Pocket CTO module boundaries, small files, thin routes, pure domain contracts, and explicit service or repository splits. Do not use for docs-only changes.
---

# Modular Architecture Guard

Pocket CTO must stay legible to both humans and agents.
Your job is to prevent the repository from turning into a pile of god files.
...
10. If you had to bend a boundary, record the reason in the active ExecPlan and an ADR if the change is architectural.

## Default split patterns

### HTTP or webhook flows
