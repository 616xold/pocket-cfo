...
- `pocket-cfo-codex-operator`
  ...
  - Skills invoked for the FP-0142 invalid-token route integration sequencing slice covering FP-0141/0140/0139/0138/0137/0136/0134/0130/0125/0107/0106/0100 boundary preservation, exact FP-0142 plan acceptance, FP-0143 absence, Authorization-present activation planning, missing-token precedence, protected-resource metadata separation, sanitized FP-0139 result-envelope-only dependency posture, app-construction co-registration prerequisites, failure taxonomy / HTTP / WWW-Authenticate consistency gates, no route integration implementation, no default buildApp wiring, no production token validation/token parser/JWT decoder/token introspection/session/OAuth/auth middleware runtime, no route expansion, no real token examples, no JWT-like examples, no Bearer token material, no token echo/logging, proof-gate bridge, direct stale-doc refresh, validation, and handoff:
  ...
  - OpenAI Developers tooling exposed only API-key setup surfaces, not read-only docs. Official web documentation was used read-only for FP-0142: MCP Authorization for invalid/expired token, insufficient-scope, protected-resource metadata, resource indicators, and token-passthrough posture; MCP Security Best Practices for no-passthrough and audience/resource posture; RFC 6750 for invalid_request / invalid_token / insufficient_scope and missing-versus-failed credentials; RFC 8707 for resource indicators/audience; RFC 9728 for protected resource metadata; OpenAI Apps SDK Authentication for MCP challenge context; and OpenAI Apps SDK Security & Privacy for no-secret-leakage and least-privilege posture. No API-key setup, OpenAI API call, model call, provider call, deployment, external communication, source mutation, finance write, public asset generation, or app-submission action was used.
