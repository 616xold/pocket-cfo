...
This file records the plugins available in this Codex local thread and the repo-local Pocket CFO plugin used for the FP-0107 local-only Fastify `POST /mcp` route-adapter shell slice.
...
- `pocket-cfo-codex-operator`
  ...
  - Skills invoked for this FP-0107 local route-adapter shell, proof-gate bridge, doc-refresh, validation, and handoff slice:
...
- OpenAI Developers was available only as an installed plugin family; no callable read-only docs tool was exposed for this slice. It was not used to create API keys, call OpenAI APIs, call models, or widen app/runtime scope. Official OpenAI web docs were used as read-only platform context for Apps SDK MCP servers and ChatGPT connector endpoint setup. Official Model Context Protocol web docs were used as read-only protocol context for JSON-RPC base behavior, `ping` liveness, and tools/list/tools/call shape.
